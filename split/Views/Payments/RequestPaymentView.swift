            ...
            Button(action: {
                ...
                if amountObj.showBulk {
                    amountObj.bulkPeople.forEach { mem in
                        ...
                        })?.name ?? "unknown recipient", reqfrom: [mem.name], amount: amountObj.bulkValues[mem.id]! * ((includeTax ? Float(taxRate) ?? 0.0 : 0.0)/100 + 1), time: Int(NSDate().timeIntervalSince1970), memo: memoText == "" ? "Generated by Bulk split" : memoText,  type: .request, special: includeSelf ? "includeself" : "") , h: house)
                    }
                } else {
                    Fetch.sendPayment(p: Payment(to: house.members.first(where: { (m) -> Bool in
                        return m.id == UserDefaults.standard.string(forKey: "myId")
                        ...
                        return m.name
                    }), amount: ((includeSelf ? (Float(amountText)! * Float(choice.count) / Float(choice.count + 1)) : (Float(amountText)!))  * ((includeTax ? (Float(taxRate) ?? 0.0) : 0.0)/100 + 1)), time: Int(NSDate().timeIntervalSince1970), memo: memoText, type:.request, special: includeSelf ? "includeself" : ""), h: house)
                }
                //clear
                includeSelf = false
                choice = [Member]()
