            ...
            Button(action: {
                ...
                if amountObj.showBulk {
                    amountObj.bulkPeople.forEach { mem in
                        ...
                        })?.name ?? "unknown recipient", reqfrom: [mem.name], amount: amountObj.bulkValues[mem.id]!, time: Int(NSDate().timeIntervalSince1970), memo: memoText == "" ? "Generated by Bulk split" : memoText, isRequest: true), h: house)
                    }
                } else {
                    Fetch().sendPayment(p: Payment(to: house.members.first(where: { (m) -> Bool in
                        return m.id == UserDefaults.standard.string(forKey: "myId")
