            ...
            Button(action: {
                ...
                if amountObj.showBulk {
                    amountObj.bulkPeople.forEach { m in
                        ...
                        })?.name ?? "unknown sender", amount: amountObj.bulkValues[m.id]! * (includeTax ? Float(taxRate) ?? 1.0 : 1.0), time: Int(NSDate().timeIntervalSince1970), memo: memoText == "" ? "Generated by Bulk split" : memoText, isRequest: false), h: house)
                    }
                } else {
                    Fetch().sendPayment(p: Payment(to: choice.first!.name, from: house.members.first(where: { (m) -> Bool in
                        return m.id == UserDefaults.standard.string(forKey: "myId")
                    })?.name ?? "unknown sender", amount: Float(amountText)! * (includeTax ? Float(taxRate) ?? 1.0 : 1.0), time: Int(NSDate().timeIntervalSince1970), memo: memoText, isRequest: false), h: house)
                }
