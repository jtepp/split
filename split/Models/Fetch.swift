...
//                print("STARTED 6")
    ...
    func deleteAllPayments(h: House) {
...
//                }
    ...
    func updatePaymentSave(payment: Payment, member: Member, nAmount: Float, nMemo: String, nTo: String, nFrom: String) {
...
func compactPayments(_ mems: [Member]) -> [Payment] {
    ...
    for i in 0..<members.count {
        ...
        for j in 0..<members.count {
            ...
            if (from.balance > 0 && to.balance < 0) {
                let amount = min(from.balance, -to.balance)
                ps.append(Payment(id: UUID().uuidString, to: to.name, reqfrom: [from.name], amount: amount, time: Int(NSDate().timeIntervalSince1970), memo: "Generated by Compactor", type: .request, special: "compactor", by: UserDefaults.standard.string(forKey: "myId") ?? "Compactor"))
                members[i].balance -= amount
                members[j].balance += amount
            }
        }
